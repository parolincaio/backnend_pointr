-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1
-- Tempo de geração: 29/05/2026 às 21:52
-- Versão do servidor: 10.4.32-MariaDB
-- Versão do PHP: 8.2.12

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Banco de dados: `pointr`
--

-- --------------------------------------------------------

--
-- Estrutura para tabela `colaborador`
--

CREATE TABLE `colaborador` (
  `id_colaborador` int(11) NOT NULL,
  `id_empresa` int(11) NOT NULL,
  `id_turno` int(11) DEFAULT NULL,
  `nome_completo` varchar(100) NOT NULL,
  `cpf` varchar(14) DEFAULT NULL,
  `email` varchar(100) NOT NULL,
  `telefone` varchar(20) DEFAULT NULL,
  `senha` varchar(255) NOT NULL,
  `avatar_img` mediumblob DEFAULT NULL,
  `biometria_facial` mediumblob DEFAULT NULL,
  `cargo` varchar(50) DEFAULT NULL,
  `status` tinyint(1) DEFAULT 1,
  `horas_extras` decimal(5,2) DEFAULT 0.00,
  `data_contratacao` date DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Despejando dados para a tabela `colaborador`
--

INSERT INTO `colaborador` (`id_colaborador`, `id_empresa`, `id_turno`, `nome_completo`, `cpf`, `email`, `telefone`, `senha`, `avatar_img`, `biometria_facial`, `cargo`, `status`, `horas_extras`, `data_contratacao`) VALUES
(1, 1, 1, 'João Silva', '123.456.789-00', 'joao@empresa.com', '(11)98888-7777', '123456', NULL, NULL, 'Vendedor', 1, 0.00, NULL);

-- --------------------------------------------------------

--
-- Estrutura para tabela `configuracoes`
--

CREATE TABLE `configuracoes` (
  `id_config` int(11) NOT NULL,
  `id_empresa` int(11) DEFAULT NULL,
  `tema_layout` varchar(30) DEFAULT NULL,
  `tema_tabela` varchar(30) DEFAULT NULL,
  `notificacoes` tinyint(1) DEFAULT 1,
  `alto_contraste` tinyint(1) DEFAULT 0,
  `atrasos` tinyint(1) DEFAULT 1,
  `backup_automatico` tinyint(1) DEFAULT 1
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `dispositivo`
--

CREATE TABLE `dispositivo` (
  `id_dispositivo` int(11) NOT NULL,
  `id_empresa` int(11) NOT NULL,
  `tipo_dispositivo` varchar(50) DEFAULT NULL,
  `codigo_dispositivo` varchar(100) DEFAULT NULL,
  `conexao` tinyint(1) DEFAULT 1
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `empresa`
--

CREATE TABLE `empresa` (
  `id_empresa` int(11) NOT NULL,
  `nome_empresa` varchar(100) NOT NULL,
  `cnpj` varchar(18) NOT NULL,
  `email` varchar(100) NOT NULL,
  `telefone` varchar(20) DEFAULT NULL,
  `endereco` varchar(200) DEFAULT NULL,
  `senha` varchar(255) NOT NULL,
  `link_acesso` varchar(200) DEFAULT NULL,
  `cod_acesso` varchar(10) DEFAULT NULL,
  `avatar` mediumblob DEFAULT NULL,
  `gasto_estimado` decimal(10,2) DEFAULT NULL,
  `nivel_assinatura` tinyint(4) NOT NULL DEFAULT 1,
  `qtd_funcionarios` int(11) DEFAULT 0,
  `data_criacao` datetime DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Despejando dados para a tabela `empresa`
--

INSERT INTO `empresa` (`id_empresa`, `nome_empresa`, `cnpj`, `email`, `telefone`, `endereco`, `senha`, `link_acesso`, `cod_acesso`, `avatar`, `gasto_estimado`, `nivel_assinatura`, `qtd_funcionarios`, `data_criacao`) VALUES
(1, 'Natura Alto Tietê', '12.345.678/0001-90', 'contato@natura.com', '(11)99999-9999', 'Mogi das Cruzes - SP', '123456', NULL, NULL, NULL, NULL, 1, 0, '2026-05-27 19:01:39'),
(4, 'empresa2', '1222222', 'teste_empresa2', NULL, NULL, '123', NULL, NULL, NULL, NULL, 1, 0, '2026-05-28 21:24:02'),
(5, 'empresa3', '333', 'empresa3', NULL, NULL, '12', NULL, NULL, NULL, NULL, 1, 0, '2026-05-29 13:11:30'),
(6, 'empresa_testes2', '222', 'empresa_testes2@gmail.com', NULL, NULL, '12', NULL, NULL, NULL, NULL, 1, 0, '2026-05-29 15:22:53'),
(7, 'empresa_testes3', '333333', 'empresa_testes3@gmail.com', NULL, NULL, '12', NULL, NULL, NULL, NULL, 2, 0, '2026-05-29 15:49:34'),
(8, 'empresa_teste4', '4444444', 'empresa_teste4@gmail.com', NULL, NULL, '12', NULL, NULL, NULL, NULL, 1, 0, '2026-05-29 16:15:34');

-- --------------------------------------------------------

--
-- Estrutura para tabela `evento`
--

CREATE TABLE `evento` (
  `id_evento` int(11) NOT NULL,
  `id_empresa` int(11) NOT NULL,
  `titulo_evento` varchar(100) DEFAULT NULL,
  `descricao` text DEFAULT NULL,
  `data_evento` date DEFAULT NULL,
  `horario_evento` time DEFAULT NULL,
  `evento_recorrente` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `log_sistema`
--

CREATE TABLE `log_sistema` (
  `id_log` int(11) NOT NULL,
  `id_ponto` int(11) DEFAULT NULL,
  `horario` datetime DEFAULT current_timestamp(),
  `tipo` varchar(50) DEFAULT NULL,
  `descricao` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `ponto`
--

CREATE TABLE `ponto` (
  `id_ponto` int(11) NOT NULL,
  `id_colaborador` int(11) NOT NULL,
  `id_dispositivo` int(11) DEFAULT NULL,
  `horario` datetime NOT NULL,
  `tipo` enum('ENTRADA','SAIDA','ALMOCO','RETORNO') NOT NULL,
  `geolocalizacao` varchar(100) DEFAULT NULL,
  `modificado` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Despejando dados para a tabela `ponto`
--

INSERT INTO `ponto` (`id_ponto`, `id_colaborador`, `id_dispositivo`, `horario`, `tipo`, `geolocalizacao`, `modificado`) VALUES
(1, 1, NULL, '2026-05-27 08:01:00', 'ENTRADA', NULL, 0);

-- --------------------------------------------------------

--
-- Estrutura para tabela `relatorio`
--

CREATE TABLE `relatorio` (
  `id_relatorio` int(11) NOT NULL,
  `id_empresa` int(11) NOT NULL,
  `data_relatorio` date DEFAULT NULL,
  `periodo` varchar(50) DEFAULT NULL,
  `tipo_relatorio` varchar(50) DEFAULT NULL,
  `json_data` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL CHECK (json_valid(`json_data`)),
  `layout_relatorio` varchar(50) DEFAULT NULL,
  `horas_trabalhadas` decimal(6,2) DEFAULT NULL,
  `salario` decimal(10,2) DEFAULT NULL,
  `valor_hora_extra` decimal(10,2) DEFAULT NULL,
  `data_envio_email` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `turno`
--

CREATE TABLE `turno` (
  `id_turno` int(11) NOT NULL,
  `titulo_turno` varchar(50) DEFAULT NULL,
  `horario_entrada` time NOT NULL,
  `horario_saida` time NOT NULL,
  `horario_almoco` time DEFAULT NULL,
  `tolerancia_horario` int(11) DEFAULT 10,
  `tolerancia_almoco` int(11) DEFAULT 10,
  `carga_horaria_semanal` decimal(5,2) DEFAULT NULL,
  `recorrencia` varchar(30) DEFAULT NULL,
  `fuso_horario` varchar(50) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Despejando dados para a tabela `turno`
--

INSERT INTO `turno` (`id_turno`, `titulo_turno`, `horario_entrada`, `horario_saida`, `horario_almoco`, `tolerancia_horario`, `tolerancia_almoco`, `carga_horaria_semanal`, `recorrencia`, `fuso_horario`) VALUES
(1, 'Turno Comercial', '08:00:00', '18:00:00', NULL, 10, 10, 44.00, NULL, NULL);

--
-- Índices para tabelas despejadas
--

--
-- Índices de tabela `colaborador`
--
ALTER TABLE `colaborador`
  ADD PRIMARY KEY (`id_colaborador`),
  ADD UNIQUE KEY `email` (`email`),
  ADD UNIQUE KEY `cpf` (`cpf`),
  ADD KEY `id_empresa` (`id_empresa`),
  ADD KEY `id_turno` (`id_turno`);

--
-- Índices de tabela `configuracoes`
--
ALTER TABLE `configuracoes`
  ADD PRIMARY KEY (`id_config`),
  ADD UNIQUE KEY `id_empresa` (`id_empresa`);

--
-- Índices de tabela `dispositivo`
--
ALTER TABLE `dispositivo`
  ADD PRIMARY KEY (`id_dispositivo`),
  ADD KEY `id_empresa` (`id_empresa`);

--
-- Índices de tabela `empresa`
--
ALTER TABLE `empresa`
  ADD PRIMARY KEY (`id_empresa`),
  ADD UNIQUE KEY `cnpj` (`cnpj`),
  ADD UNIQUE KEY `email` (`email`),
  ADD UNIQUE KEY `link_acesso` (`link_acesso`),
  ADD UNIQUE KEY `cod_acesso` (`cod_acesso`);

--
-- Índices de tabela `evento`
--
ALTER TABLE `evento`
  ADD PRIMARY KEY (`id_evento`),
  ADD KEY `id_empresa` (`id_empresa`);

--
-- Índices de tabela `log_sistema`
--
ALTER TABLE `log_sistema`
  ADD PRIMARY KEY (`id_log`),
  ADD KEY `id_ponto` (`id_ponto`);

--
-- Índices de tabela `ponto`
--
ALTER TABLE `ponto`
  ADD PRIMARY KEY (`id_ponto`),
  ADD KEY `id_colaborador` (`id_colaborador`),
  ADD KEY `id_dispositivo` (`id_dispositivo`);

--
-- Índices de tabela `relatorio`
--
ALTER TABLE `relatorio`
  ADD PRIMARY KEY (`id_relatorio`),
  ADD KEY `id_empresa` (`id_empresa`);

--
-- Índices de tabela `turno`
--
ALTER TABLE `turno`
  ADD PRIMARY KEY (`id_turno`);

--
-- AUTO_INCREMENT para tabelas despejadas
--

--
-- AUTO_INCREMENT de tabela `colaborador`
--
ALTER TABLE `colaborador`
  MODIFY `id_colaborador` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- AUTO_INCREMENT de tabela `configuracoes`
--
ALTER TABLE `configuracoes`
  MODIFY `id_config` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `dispositivo`
--
ALTER TABLE `dispositivo`
  MODIFY `id_dispositivo` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `empresa`
--
ALTER TABLE `empresa`
  MODIFY `id_empresa` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;

--
-- AUTO_INCREMENT de tabela `evento`
--
ALTER TABLE `evento`
  MODIFY `id_evento` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `log_sistema`
--
ALTER TABLE `log_sistema`
  MODIFY `id_log` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `ponto`
--
ALTER TABLE `ponto`
  MODIFY `id_ponto` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- AUTO_INCREMENT de tabela `relatorio`
--
ALTER TABLE `relatorio`
  MODIFY `id_relatorio` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `turno`
--
ALTER TABLE `turno`
  MODIFY `id_turno` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- Restrições para tabelas despejadas
--

--
-- Restrições para tabelas `colaborador`
--
ALTER TABLE `colaborador`
  ADD CONSTRAINT `colaborador_ibfk_1` FOREIGN KEY (`id_empresa`) REFERENCES `empresa` (`id_empresa`),
  ADD CONSTRAINT `colaborador_ibfk_2` FOREIGN KEY (`id_turno`) REFERENCES `turno` (`id_turno`);

--
-- Restrições para tabelas `configuracoes`
--
ALTER TABLE `configuracoes`
  ADD CONSTRAINT `configuracoes_ibfk_1` FOREIGN KEY (`id_empresa`) REFERENCES `empresa` (`id_empresa`);

--
-- Restrições para tabelas `dispositivo`
--
ALTER TABLE `dispositivo`
  ADD CONSTRAINT `dispositivo_ibfk_1` FOREIGN KEY (`id_empresa`) REFERENCES `empresa` (`id_empresa`);

--
-- Restrições para tabelas `evento`
--
ALTER TABLE `evento`
  ADD CONSTRAINT `evento_ibfk_1` FOREIGN KEY (`id_empresa`) REFERENCES `empresa` (`id_empresa`);

--
-- Restrições para tabelas `log_sistema`
--
ALTER TABLE `log_sistema`
  ADD CONSTRAINT `log_sistema_ibfk_1` FOREIGN KEY (`id_ponto`) REFERENCES `ponto` (`id_ponto`);

--
-- Restrições para tabelas `ponto`
--
ALTER TABLE `ponto`
  ADD CONSTRAINT `ponto_ibfk_1` FOREIGN KEY (`id_colaborador`) REFERENCES `colaborador` (`id_colaborador`),
  ADD CONSTRAINT `ponto_ibfk_2` FOREIGN KEY (`id_dispositivo`) REFERENCES `dispositivo` (`id_dispositivo`);

--
-- Restrições para tabelas `relatorio`
--
ALTER TABLE `relatorio`
  ADD CONSTRAINT `relatorio_ibfk_1` FOREIGN KEY (`id_empresa`) REFERENCES `empresa` (`id_empresa`);
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
